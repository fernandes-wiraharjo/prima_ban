...
function redirectToIndexAdd() {
  window.location.href = '/transaction/tanda-terima/get/add';
}
...
$(function () {
  ...
  var dt_table = $('.datatables-tanda-terima'),
    statusObj = {
  ...
  if (dt_table.length) {
    var dt = dt_table.DataTable({
      ...
      serverSide: true,
      ajax: {
        url: '/transaction/tanda-terima/get',
        type: 'GET',
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            var $id = full['id'];
            var $receiverName = full['receiver_name'];
            return (
      ...
      buttons: [
        {
          text: '<i class="bx bx-plus me-0 me-sm-1"></i><span class="d-none d-sm-inline-block">Add Tanda Terima</span>',
  ...
  $('.datatables-tanda-terima tbody').on('click', '.delete-record', function () {
    ...
    if (
      ...
      $.ajax({
        url: '/transaction/tanda-terima/' + id,
        type: 'DELETE',
        headers: {
          'X-CSRF-TOKEN': token
        },
