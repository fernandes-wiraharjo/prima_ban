...
@section('page-script')
<script src="{{asset('assets/js/master-product-list.js')}}"></script>
@endsection
...
<div class="card">
  ...
  <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasAdd" aria-labelledby="offcanvasAddLabel">
    <div class="offcanvas-header">
      <h5 id="offcanvasAddLabel" class="offcanvas-title">Add Product</h5>
      <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body mx-0 flex-grow-0">
      <form class="add-new pt-0" id="addNewForm" onsubmit="return false" action="{{ route('add-product') }}" method="POST">
        @csrf
        <div class="mb-3">
        ...
        <div class="mb-3">
          ...
          <select id="brand" name="id_brand" class="select2 selectBrand form-select">
            <option value="">Select</option>
        ...
        <div class="mb-3">
          ...
          <select id="pattern" name="id_pattern" class="select2 selectPattern form-select">
            <option value="">Select</option>
          </select>
        </div>
        <div class="mb-3">
          <label class="form-label" for="uom">UOM</label>
          <select id="uom" name="id_uom" class="select2 selectUOM form-select">
            <option value="">Select</option>
            @foreach($uoms as $id => $code)
  ...
  <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasEdit" aria-labelledby="offcanvasEditLabel">
    <div class="offcanvas-header">
        <h5 id="offcanvasEditLabel" class="offcanvas-title">Edit Product</h5>
        <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body mx-0 flex-grow-0">
        <form class="edit-form pt-0" id="editForm" onsubmit="return false" action="{{ route('edit-product', ['id' => 0]) }}" method="POST">
        @method('PUT')
        @csrf
            ...
            <div class="mb-3">
              ...
              <select id="edit-brand" name="id_brand" class="select2 selectBrand form-select">
                <option value="">Select</option>
                @foreach($brands as $id => $name)
                  <option value="{{ $id }}">{{ $name }}</option>
                @endforeach
            ...
            <div class="mb-3">
              <label class="form-label" for="edit-pattern">Pattern</label>
              <select id="edit-pattern" name="id_pattern" class="select2 selectPattern form-select">
                <option value="">Select</option>
              </select>
            </div>
            <div class="mb-3">
              <label class="form-label" for="edit-uom">UOM</label>
              <select id="edit-uom" name="id_uom" class="select2 selectUOM form-select">
                <option value="">Select</option>
                @foreach($uoms as $id => $code)
                  <option value="{{ $id }}">{{ $code }}</option>
                @endforeach
